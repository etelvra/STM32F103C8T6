...
uint16_t _RTC_Time[] = {2024, 2, 14, 19, 06, 32};	//定义全局的时间数组，数组内容分别为年、月、日、时、分、秒
...
void _RTC_Init(void)
{
	...
	if (BKP_ReadBackupRegister(BKP_DR1) != 0x0805)			//通过写入备份寄存器的标志位，判断RTC是否是第一次配置
															//if成立则执行第一次的RTC配置
	{
		...
		BKP_WriteBackupRegister(BKP_DR1, 0x0805);			//在备份寄存器写入自己规定的标志位，用于判断RTC是不是第一次执行配置
	}
	else													//RTC不是第一次配置
	{
		RTC_WaitForSynchro();								//等待同步
